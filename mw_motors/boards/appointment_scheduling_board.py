...
class AppointmentSchedulingBoard(models.TransientModel):
	_name = 'appointment.scheduling.board'
	_description = 'Appointment Scheduling Board'
	...
	def _check_repair_order(self, dddd, start_time, stall_id):
		gaps = 4
		ro = self.env['car.repair.order'].search([
			# ('state','=','open'),
			('stall_id','=',stall_id),
			('scheduled_date','=',dddd),
			('scheduled_time','>=',start_time),
			('scheduled_time','<',start_time+1)], limit=1)
		if ro:
			sd = ro.date_reception+timedelta(hours=8)
			sd = sd.strftime("%m/%d өдөр %H:%M")
			ed = ro.date_delivery+timedelta(hours=8)
			ed = ed.strftime("%m/%d өдөр %H:%M")
			info = {
				'id': ro.id,
				'name': ro.name,
				...
				'start_cell_number': self._get_start_cell_number(ro.scheduled_time),
				'date_reception': sd,
				'date_delivery': ed,
				'staff_name': ro.assigned_user_id.display_name or ""
			}
			gaps = self._compute_gaps(ro.spend_time)
			return gaps, info
		else:
			return gaps, False
	...
	def get_timetable_datas(self, dddd, context=None):
		...
		for ll in lines:
			...
			for st in range(start_time, end_time+1):
				print('==---------time: ', st, start_idx, end_idx)
				gaps, ro_info = self._check_repair_order(dddd, st, ll.stall_id.id)
				# Ordered cell =========================
				if ro_info:
					cell = {}
					# Сул cell бөглөх
					if ro_info['start_cell_number'] > 1:
						...
						temp_idx = merge 
						print('===ordered 01: ', start_idx, end_idx, merge)
					cell['ro_id'] = ro_info['id']
					cell['ro_name'] = ro_info['name']
					cell['state_number'] = ro_info['state_number']
					# Tooltip
					desc = "%s - %s %s |%s, %s" % (ro_info['date_reception'],ro_info['date_delivery'],ro_info['staff_name'],ro_info['customer_name'],ro_info['car_model'],)
					...
					print('===ordered 02: ', start_idx, end_idx, gaps)
					# Сул cell бөглөх
					start_idx += ro_info['start_cell_number']+gaps
					if start_idx <= end_idx:
						merge = end_idx-start_idx
						print('===ordered 03: ', start_idx, end_idx, merge+1)
						cells.append({'state':'empty', 'value_colspan': merge+1}) 
						start_idx += merge
						
					print('===ordered 04: ', start_idx, end_idx)

				# Blank cell =============================
				elif ll._is_blank(day_name, st):
					total_work_time -= 1
					# Сул cell бөглөх
					print('===blank', start_idx, end_idx)
				...
				# Empty cell ===========================
					...
					# Сул cell бөглөх
					if start_idx <= end_idx:
						print('===empty', start_idx, end_idx)
						temp = {
							'state':'empty', 
							'value_colspan': end_idx+1-start_idx, 
							'stall_id': ll.stall_id.id,
							'time': st, 
						}
						cells.append(temp)
						start_idx = end_idx+1
		...
		datas['timetable_lines'] = line_datas
